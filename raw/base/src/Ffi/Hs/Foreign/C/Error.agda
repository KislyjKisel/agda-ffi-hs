...
postulate
    ...
    throwErrnoIf-discard                    : (A → Bool) → List Char → IO A → IO (⊤ {lzero})
    throwErrnoIfRetry                       : (A → Bool) → List Char → IO A → IO A
    throwErrnoIfRetry-discard               : (A → Bool) → List Char → IO A → IO (⊤ {lzero})

    throwErrnoIfMinus1                      : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → IO A → IO A
    throwErrnoIfMinus1-discard              : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → IO A → IO (⊤ {lzero})
    throwErrnoIfMinus1Retry                 : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → IO A → IO A
    throwErrnoIfMinus1Retry-discard         : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → IO A → IO (⊤ {lzero})
    ...
    throwErrnoIfRetryMayBlock-discard       : (A → Bool) → List Char → IO A → IO B → IO (⊤ {lzero})

    throwErrnoIfMinus1RetryMayBlock         : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → IO A → IO B → IO A
    throwErrnoIfMinus1RetryMayBlock-discard : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → IO A → IO B → IO (⊤ {lzero})
    ...
    throwErrnoPathIf-discard                : (A → Bool) → List Char → List Char → IO A → IO (⊤ {lzero})
    ...
    throwErrnoPathIfMinus1-discard          : ⦃ Eq A ⦄ → ⦃ Num A ⦄ → List Char → List Char → IO A → IO (⊤ {lzero})

{-# COMPILE GHC throwErrnoIf                    = \ aℓ a -> Foreign.C.Error.throwErrnoIf       #-}
{-# COMPILE GHC throwErrnoIf-discard            = \ aℓ a -> Foreign.C.Error.throwErrnoIf_      #-}
{-# COMPILE GHC throwErrnoIfRetry               = \ aℓ a -> Foreign.C.Error.throwErrnoIfRetry  #-}
{-# COMPILE GHC throwErrnoIfRetry-discard       = \ aℓ a -> Foreign.C.Error.throwErrnoIfRetry_ #-}

{-# COMPILE GHC throwErrnoIfMinus1              = \ aℓ a AgdaEq AgdaNum -> Foreign.C.Error.throwErrnoIfMinus1       #-}
{-# COMPILE GHC throwErrnoIfMinus1-discard      = \ aℓ a AgdaEq AgdaNum -> Foreign.C.Error.throwErrnoIfMinus1_      #-}
{-# COMPILE GHC throwErrnoIfMinus1Retry         = \ aℓ a AgdaEq AgdaNum -> Foreign.C.Error.throwErrnoIfMinus1Retry  #-}
{-# COMPILE GHC throwErrnoIfMinus1Retry-discard = \ aℓ a AgdaEq AgdaNum -> Foreign.C.Error.throwErrnoIfMinus1Retry_ #-}
...
{-# COMPILE GHC throwErrnoIfRetryMayBlock-discard = \ aℓ a bℓ b -> Foreign.C.Error.throwErrnoIfRetryMayBlock_ #-}

{-# COMPILE GHC throwErrnoIfMinus1RetryMayBlock         =
    \ aℓ a bℓ b AgdaEq AgdaNum -> Foreign.C.Error.throwErrnoIfMinus1RetryMayBlock  #-}
{-# COMPILE GHC throwErrnoIfMinus1RetryMayBlock-discard =
...
{-# COMPILE GHC throwErrnoPathIf-discard      = \ aℓ a      -> Foreign.C.Error.throwErrnoPathIf_             #-}
...
{-# COMPILE GHC throwErrnoPathIfMinus1-discard = \ aℓ a AgdaEq AgdaNum -> Foreign.C.Error.throwErrnoPathIfMinus1_ #-}
